      ...
      <div className="flex items-center justify-between px-4 py-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="#/" className="relative">
            ...
            <span className="absolute -top-2 -right-2 bg-blue-500 text-[10px] font-bold px-1.5 py-0.5 rounded-full">
              3
            </span>
          </Link>
          <button onClick={() => setIsSearchOpen(!isSearchOpen)}>
            <FaSearch
              size={20}
              className={isSearchOpen ? "text-green-500" : ""}
            />
          </button>
        ...
        <div className="fixed inset-0 bg-black/60 z-40" onClick={toggleMenu}>
          ...
          >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.name} href={link.href} onClick={toggleMenu}>
                    <li
                      className={`text-lg py-2 px-3 rounded-lg ${isActive ? "text-green-500 font-bold bg-zinc-900" : "text-zinc-300"}`}
                    >
                      {link.name}
                    </li>
                  </Link>
            ...
            <Link
              href="#products"
              onClick={toggleMenu}
              className="w-full py-3 bg-green-600 text-center font-bold rounded-lg"
            >
              Shop Collection
            </Link>
